...
-- PK - 21
-- Title - Meal Flag Details by Employees


PREPARE usrrptplan (date, date) AS
...
WHERE ("activity_activitysplit"."date" >= $1
       ...
       AND (CASE
            ...
            END)
       AND ( CASE
                 WHEN '$$employee_id$$' <> 0 THEN "activity_activitysplit"."employee_id" = $$employee_id$$
                 ELSE 1=1
             END )
       AND (CASE
...
EXECUTE usrrptplan($$start_date$$, $$end_date$$);